...
GEOCODE_UA = os.getenv("GEOCODE_UA", "tg-broker-bot/2.1 (contact: set-your-email@example.com)")
...
def main_menu_markup():
    return ReplyKeyboardMarkup(
        [[BTN_NEW_REQUEST, BTN_CATALOG], [BTN_HELP, BTN_MENU]], resize_keyboard=True
    )
...
async def role_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Я заказчик":
        ...
        await update.message.reply_text(
            "Готово. Вы можете создать заявку или открыть каталог.",
        ...
        await update.message.reply_text(
            "Вы — исполнитель. Админ добавит вас по @username или числовому ID.\n"
            "Важно: чтобы получать заявки, один раз нажмите /start у бота.",
            reply_markup=main_menu_markup()
        )
        return ConversationHandler.END
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Если запутались — нажмите «В начало».",
        reply_markup=main_menu_markup()
    )
...
async def mode_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == BTN_CANCEL:
        await update.message.reply_text("Отменено.", reply_markup=main_menu_markup())
        return ConversationHandler.END
...
async def radius_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mode = context.user_data.get("req_mode","auction")
    if mode == "auction":
        await update.message.reply_text(f"Заявка #{req_id} создана. Рассылаю исполнителям…", reply_markup=main_menu_markup())
        candidates = await find_candidates(req_id)
        if not candidates:
            await update.message.reply_text("Подходящих исполнителей не найдено. Админ будет уведомлён.", reply_markup=main_menu_markup())
        else:
            for row in candidates:
                ...
                try:
                    ...
                    pass
            await update.message.reply_text("Как только поступят офферы — я пришлю вам.", reply_markup=main_menu_markup())
        return ConversationHandler.END
    else:
        candidates = await find_candidates(req_id)
        if not candidates:
            await update.message.reply_text("Исполнителей в радиусе не найдено.", reply_markup=main_menu_markup())
            return ConversationHandler.END
...
async def on_request_offer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not req or not ex:
        await q.message.reply_text("Не удалось отправить запрос — проверьте наличие заявки/исполнителя.", reply_markup=main_menu_markup())
        return
    ...
    if ok:
        await q.message.reply_text(f"Запрос оффера отправлен исполнителю E-{exid:05d}.", reply_markup=main_menu_markup())
    else:
        await q.message.reply_text("Не удалось доставить запрос. Исполнитель мог не запускать бота.", reply_markup=main_menu_markup())
...
async def on_accept_offer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        row = await cur.fetchone()
    if not row:
        await q.message.reply_text("Оффер не найден.", reply_markup=main_menu_markup())
        return
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sub == "prefer_owner" and len(args)>=2:
        ...
        await update.message.reply_text(f"prefer_owner_first = {v}", reply_markup=main_menu_markup())
        ...
        if not rows:
            await update.message.reply_text("Исполнителей нет.", reply_markup=main_menu_markup())
            return
        lines = []
        ...
        await update.message.reply_text("\n".join(lines)[:4000], reply_markup=main_menu_markup())
    elif sub == "set_loc" and len(args)>=2:
        context.user_data["await_loc_for_exec"] = int(args[1])
        await update.message.reply_text("Окей. Отправьте геолокацию сообщением-ответом.", reply_markup=main_menu_markup())
    elif sub == "assign" and len(args)>=3:
        rid = int(args[1]); exid = int(args[2])
        req = await get_request(rid)
        ex = await get_executor(exid)
        if not req or not ex:
            await update.message.reply_text("Проверьте request_id и executor_id.", reply_markup=main_menu_markup())
            return
        _, _, category, desc, addr, city, lat, lon, crad, mode, status = req
        text = (
...
async def on_location_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.location:
        await update.message.reply_text("Нужна геолокация.", reply_markup=main_menu_markup())
        return
    ...
    await update.message.reply_text(f"Локация исполнителя E-{exid:05d} обновлена.", reply_markup=main_menu_markup())
...
def build_app():
    ...
    # Text buttons to open menu/help as well
