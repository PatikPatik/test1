...
GEOCODE_UA = os.getenv("GEOCODE_UA", "tg-broker-bot/inline-only/1.0 (contact: set-your-email@example.com)")
...
async def on_imenu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "home":
        ...
        await q.message.reply_text(
            "Как пользоваться:\n"
            "— «Создать заявку» → выберите режим → категория → краткое ТЗ → адрес → радиус.\n"
            "— «Каталог» → сразу выбираете исполнителя из списка и запрашиваете оффер.\n"
            "— «Мои заявки» → список + офферы.\n"
            "— Контакты открываются после принятия оффера.\n",
            reply_markup=inline_main_menu()
        )

# --- Client: new request flow (all inline)
...
async def desc_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Укажите адрес (город, улица, дом; можно ориентиры).", reply_markup=inline_cancel())
    return ADDR_IN

async def addr_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    addr = update.message.text.strip()
    context.user_data["req_addr"] = addr
    msg = await update.message.reply_text("Ищу адрес…")
    results = await geocode_address(addr)
    if not results:
        await msg.edit_text("Не нашёл адрес. Попробуйте написать по-другому.", reply_markup=inline_cancel())
        return ADDR_IN
    ...
    await msg.edit_text("Выберите подходящий вариант:", reply_markup=InlineKeyboardMarkup(buttons))
    return GEO_PICK
...
async def on_geo_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text("Радиус поиска, км (например 50):", reply_markup=inline_cancel())
    return RAD_IN
...
async def radius_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("Введите число км от 1 до 1000.", reply_markup=inline_cancel())
        return RAD_IN
    ...
    if mode == "auction":
        await update.message.reply_text(f"Заявка #{req_id} создана. Рассылаю исполнителям…", reply_markup=after_kb)
        ...
        if candidates:
            ...
            await update.message.reply_text("Подходящих исполнителей не найдено.", reply_markup=after_kb)
        return ConversationHandler.END
    else:
        candidates = await find_candidates(req_id)
        if not candidates:
